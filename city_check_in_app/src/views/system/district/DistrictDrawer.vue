  ...
  >
    <el-form
      ...
      <el-form-item label="区县名称" prop="name">
        <el-input v-model="drawerProps.rowData.name" placeholder="请填写区县名称" clearable/>
      </el-form-item>

      <el-form-item label="区县代码" prop="code">
        <el-input v-model="drawerProps.rowData.code" placeholder="请填写区县代码" clearable/>
      </el-form-item>

      <el-form-item label="区县描述" prop="description">
        <el-input v-model="drawerProps.rowData.description" placeholder="请填写区县描述" clearable type="textarea" :rows="3"/>
      </el-form-item>
...
<script setup lang="ts" name="DistrictDrawer">
  ...
  const handleSubmit = () => {
    ruleFormRef.value!.validate(async (valid) => {
      ...
      try {
        loading.value = true;
        // 处理接口参数
        const { name, code, description } = drawerProps.value.rowData;
        let params: any = { name, code, description };
        if (drawerProps.value.title === '编辑') {
          params.id = drawerProps.value.rowData.id;
        }
        await drawerProps.value.api!(clearEmptyValue(params));
        ElMessage.success({ message: `${drawerProps.value.title}区县成功！` });
