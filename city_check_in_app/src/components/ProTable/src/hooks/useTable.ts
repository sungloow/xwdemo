...
export const useTable = (
  ...
  const getTableList = async () => {
    try {
      ...
      // 解构后台返回的分页数据
      const { pageNumber, pageSize, totalRow } = data;
      // 分页信息有的接口并没有携带，需要做一个兼容
      const page = pageNumber ? pageNumber : state.pageable.pageNumber;
      const limit = pageSize ? pageSize : state.pageable.pageSize;
      // 如果有分页更新分页信息
      isPageable && updatePageable({ pageNumber: page, pageSize: limit, total: totalRow });
    } catch (error) {
      state.loading = false;
    }
  };
